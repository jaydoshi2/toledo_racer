...
function SidebarNav() {
  ...
  const currentStepNumber =
    steps.findIndex((step) => pathname.includes(step.path)) + 1 || (pathname === "/create_model" ? 1 : 0)

  return (
    <div>
      {steps.map((step) => {
        // A step is active if it's the current step
        const isActive = pathname.includes(step.path) || (pathname === "/create_model" && step.number === 1)

        // A step is completed if its number is less than the current step
        const isCompleted = step.number < currentStepNumber

        // A step is accessible if it's completed or it's the current step
        ...
        return (
          <div key={step.number} className="mb-6">
            ...
            <Link href={isAccessible ? step.path : "#"}>
              <div
